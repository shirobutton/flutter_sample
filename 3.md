...
魔理沙: そうだぜ
Notifierは、Riverpod 2.0では新たに追加された変更を通知するためのオブジェクトだぜ。
...
霊夢:StateProviderと同じで
stateに代入するとstateの値が更新されるのね
つまりincrementを実行するとstateが1増えるっぽいね
...
NotifierがStateProviderにかなり似てるってところが分かってもらえたらいいぜ
...
ここで問題だが、shared_preferencesは基本的には文字列、数値、ブーリアン、文字列のリストしか保存できない。
...
魔理沙：そうだぜ、ただこれだけだとデータの変更ができないからデータの変更できるようにするぜ
...
データベースの作成はexecuteを使ってCREATE TABLEをそのまま入力しているぜ

ほかにも指定したバージョンが以前に開いたものよりも新しい場合に呼ぶonUpgradeがあったりするぜ

霊夢: うーん。まぁなんとなくわかったよ。とりあえずデータベースを開いてテーブルを作ってるんだね。

魔理沙： ああ結局のところはそうだぜ
じゃあ次はTaskデータクラスを編集していくぜ。
SQLiteではbool値は0と1で表現されるぜ、それにデータベースで使うためのidも追加するぜ
...
魔理沙: そうだぜ、Jsonでsqfliteの関数に渡すといい感じにしてくれるぜ
...
魔理沙: じゃあ次はTaskListNotifierを編集してデータベースを使うように変更するぜ。
...
build内で非同期処理を行えるぜ、今回はデータベースからデータを取得しているぜ。
...
第一話でも少ししたが、FutureProviderの戻り値はWidgetでも使いやすいAsyncValueというもので返されるんだぜ
...
これはその名の通りrivepodのジェネレーターだぜ。短いコードからProviderを生成してくれるぜ。
